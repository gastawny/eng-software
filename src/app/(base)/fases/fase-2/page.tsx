...
export default function Phase2Page() {
  ...
  return (
    <div>
      <Dialog open={dialog.modal} onOpenChange={() => setDialog(curr => ({...curr, modal: !curr.modal}))}>
        <DialogContent className="gap-6 p-8 w-[48rem] ">
          {click.length == 0 && <div className="flex justify-center items-center flex-col gap-6">
          ...
          <DialogFooter>
            ...
            {click.length > 0 && <Button onClick={handleClick} size='lg'>Confirmar</Button>}
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <div className='flex absolute left-1/2 bottom-10 2xl:bottom-32 -translate-x-1/2 w-4/5 gap-6 justify-between'>
        {options.map((option, index) => (
          <button onClick={() => {setDialog({modal: true}); setClick(option.label)}} key={index} className='flex flex-col shadow-md hover:bg-primary/20 duration-150 bg-card py-4 2xl:py-6 px-8 2xl:px-12 rounded-xl items-center justify-between gap-4 2xl:gap-6'>
            <p className='text-xl font-semibold'>{option.label}</p>
            <div className='bg-primary/50 p-4 rounded-xl'>
              <img className='w-10 2xl:w-16 h-10 2xl:h-16' src={`/assets/svgs/${option.label}.svg`} />
            </div>
          </button>
        ))}
      </div>
      <div className="absolute left-1/2 bottom-1 2xl:bottom-4 -translate-x-1/2 w-1/3 2xl:w-1/2 text-center">
        <Progress
          ...
          value={indexQuestion / 3 * 100}
        />
        <p className="text-xl 2xl:mt-2">
          {(indexQuestion / 3 * 100).toFixed(0)}%
            completo
        </p>
      </div>
    </div>
